...
<body>
	<div style="width: 90%; margin: 0 auto; text-align: center;" class="card shadow">
		...
		%> 
						<input class="btn mb-2 btn-secondary" type="button" value="글수정" onclick="location.href='adminIndex.jsp?pages=editNotice&n_num=<%= n_num %>&pageNum=<%=pageNum%>'">
						<input class="btn mb-2 btn-secondary" type="button" value="글삭제" onclick="location.href='deleteNotice.jsp?n_num=<%= n_num %>&pageNum=<%=pageNum%>'">
		<%
					}
				}
		%>
					<input class="btn mb-2 btn-secondary" type="button" value="글목록" onclick="location.href='adminIndex.jsp?pages=listNotice&pageNum=<%=pageNum%>'">
				</td>
			</tr>
		</table>
	<%
	...
	%>
				<div style="margin-bottom: 20px">
					<a class="btn mb-2 btn-secondary" href="adminIndex.jsp?pages=showNotice&n_num=<%= n_after %>&pageNum=<%= pageNum %>" id="upPage">
						이전글 [<%= n_title2 %>]
					</a>
				</div>
	<%
	...
	%>
			<div style="margin-bottom: 20px">
				<a class="btn mb-2 btn-secondary" href="adminIndex.jsp?pages=showNotice&n_num=<%= n_before %>&pageNum=<%= notice.getBefore() %>" id="upPage">
					다음글 [<%= n_title2 %>]
				</a>
			</div>
	<%
