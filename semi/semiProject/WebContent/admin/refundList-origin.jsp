...
<%
	...
	ArrayList<OrderManageBean> list = omdb.orderList(startRow,pageSize,"Y");
...
<body>
	<center>
		<h1>환불 관리</h1>
		<form action="refund_ok.jsp&pageNum=<%= pageNum %>" method="post">
		<table border="1">
			<tr>
				<td>주문 번호</td>
				<td>제품 번호</td>
				...
				<td>금액</td>
				<td>주문 상태</td>
				<td>환불 처리</td> <!-- 라디오 버튼  > 수정 버튼 > 정보 수정, 환불 처리 -->
			</tr>
	<%
	...
	%>
			...
			onmouseout="this.style.backgroundColor='#E8E8E8'">
				...
				<td>
					<label for="refund_n" style=cursor:pointer;>취소</label>
					<input type="checkbox" id="refund_n" name="refund" value="취소" style=display:none; />
					<label for="refund_y" style=cursor:pointer;>승인</label>
					<input type="checkbox" id="refund_y" name="refund" value="승인" style=display:none; />
				</td>
	<%			
		}
	%>
