...
public class NoticeBean {
	...
	public static String pageNumber(int limit) {
		...
		if(startPage > 1) {
			...
			str="<a href = 'admin_main.jsp?page=listNotice.jsp&pageNum="+(temp3)+"'>[이전]</a>&nbsp;&nbsp";
		}
		for(int i=startPage;i<(startPage+limit);i++) {
			if(i==pageNum) {
				str +="["+i+"]&nbsp&nbsp";
			} else {
				str+="<a href='admin_main.jsp?page=listNotice.jsp&pageNum="+i+"'>["+i+"]</a>&nbsp;&nbsp;";
			}
			if(i>=pageCount)break;
		}
		
		if(startPage + limit<=pageCount) {
			str+="<a href = 'admin_main.jsp?page=listNotice.jsp&pageNum="+(startPage+limit)+"'>[다음]</a>&nbsp;&nbsp";
		}
